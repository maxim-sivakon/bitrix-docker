...
RUN yum -y install epel-release \
    ...
    yum-utils && \
    yum-config-manager remi-php74 && \
    yum-config-manager --enable remi-php81 && \
    yum-config-manager remi-php82 && \
    yum-config-manager remi-php83
...
RUN wget http://nginx.org/download/nginx-1.25.0.tar.gz && tar xzvf nginx-1.25.0.tar.gz && \
    cd nginx-1.25.0 && \
